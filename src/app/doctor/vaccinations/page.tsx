...
const columns: ColumnDef<Vaccination>[] = [
  ...
  { accessorKey: "dateAdministered", header: "Tanggal", cell: ({ row }) => new Date(row.original.dateAdministered).toLocaleDateString("id-ID") },
  { accessorKey: "nextDueDate", header: "Berikutnya", cell: ({ row }) => row.original.nextDueDate ? new Date(row.original.nextDueDate).toLocaleDateString("id-ID") : "-" },
];
...
export default function DoctorVaccinationsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Vaksinasi" description="Daftar vaksinasi pasien" />
